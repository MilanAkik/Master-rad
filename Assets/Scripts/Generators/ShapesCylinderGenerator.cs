...
namespace Assets.Scripts.Generators
{
    ...
    public class ShapesCylinderGenerator : CylinderGenerator
    {
        ...
        private Matrix4x4[] GenerateCubeCylinders(GeneratorParameters parameters)
        {
            ...
            var areaMin = parameters.AreaMin;
            var areaMax = parameters.AreaMax;
            var matrices = new Matrix4x4[count];
            int i = 0;
            for (; i * i * i < count; i++);
            float size = (float)i;
            var xSize = areaMax.x - areaMin.x;
            var zSize = areaMax.z - areaMin.z;
            var radMod = (xSize < zSize ? xSize : zSize);
            var hMod = areaMax.y - areaMin.y;
            for (int x = 0; x < size; x++)
            {
                for(int y = 0; y < size; y++)
                {
                    for(int z = 0; z < size; z++)
                    {
                        ...
                        {
                            float posX = Mathf.Lerp(areaMin.x, areaMax.x, (x+0.5f) / size);
                            float posY = Mathf.Lerp(areaMin.y, areaMax.y, y / size);
                            float posZ = Mathf.Lerp(areaMin.z, areaMax.z, (z+0.5f) / size);
                            float h = (1f / size) * hMod * parameters.HeightMultiplier;
                            float r = (0.5f / size) * radMod * parameters.RadiusMultiplier;
